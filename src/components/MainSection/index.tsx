...
export default function MainSection(): React.ReactNode {
    ...
    const { data, isFetching } = useGetAllOS();
    const { deleteInfringement } = useDeleteOS();
    ...
    return (
        <main className={styles.mainSection}>
            {isFetching ? <h2 className={styles.fetchingWhenTrue}>Carregando todas as OS...</h2> :
                    ...
                    <tbody>
                        {
                            ...
                            :
                            ArrayWithDatas?.slice().reverse().map((os) => (
                                <tr key={os.id}>
                                    <td>{os.clientName}</td>
                                    ...
                                    <td>
                                        <ul>
                                            ...
                                            <li onClick={() => { fetchDataById(os.id, 'editSection') }}> {/* Pencil icon to the action: Edit*/}
                                            ...
                                            <li onClick={() => { deleteInfringement(os.id) }}> {/* Trash icon to the action: Delete*/}
